...
penguins |>
  ...
  geom_segment(data=penguin_means,
               aes(x = mean_bill_length, xend = mean_bill_length,
               y = as.numeric(species) - 0.2, yend = as.numeric(species) + 0.2), 
...
penguins |>
  mutate(species = fct_relevel(species, "Adelie", "Gentoo", "Chinstrap")) |>
  ggplot(aes(x = bill_length_mm, y = species)) +
  geom_jitter(aes(color=species), height = .18, alpha = .5) +
  geom_segment(data=penguin_means,
               aes(x = mean_bill_length, xend = mean_bill_length,
               y = as.numeric(species) - 0.2, yend = as.numeric(species) + 0.2), 
               color = "black", linewidth = 1)+
...
- Y axis is hard for some audiences to interpret
...
penguins |>
  ...
  geom_jitter(width = 0.3, alpha = 0.4, color = "black", size = 1) +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "Bill Length of Chinstrap, Gentoo, & Adelie Penguins",
       x = "Density",
...
:::{.extra-small-font}
- Add a trendline and regression equation
...
:::{.extra-small-font}
- Add an overall trendline and regression equation
...
penguins |>
  ggplot(aes(x = bill_length_mm, y = flipper_length_mm, color=species)) +
  geom_point(aes(size=body_mass_g), alpha = .7) +
  stat_regline_equation(aes(color=NULL))+
