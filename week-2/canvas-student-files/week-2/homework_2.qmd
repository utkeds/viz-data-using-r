...
Before we can use a package, we need to install it. We can install it using `install.packages("package_name")`. We only need to run this the first time we install a package, and you typically install them in your console rather than in your R script or Quarto document.
...
Do so now (in the console), if you have yet to install tidyverse. You probably already have in previous tasks so far in this class.

## Load Packages

Everytime we use a package, we need to load it.
...
ggplot(data = gapminder_cleaned, aes(x = gdpPercap, y = lifeExp)) +
  ...
  geom_point(aes(size = pop, fill = continent), shape = 21, color = "black") +
  ...
  scale_x_log10(breaks = c(0500, 1000, 2000, 4000, 8000, 16000, 32000, 64000)) +
  ...
  labs(
    ...
    subtitle = paste0(
      "In the following visualization you can see the relationship between income and life expectancy among all countries in 2007.",
