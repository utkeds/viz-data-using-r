...
highered_top_5 <- tidykids %>%
  ...
  ______(year %in% c(2016,2015,2014)) %>%
...
three_categories <- tidykids %>%
  filter(variable ______ c("pubhealth", "PK12ed", "parkrec")) %>%
  filter(state ______ c("Florida", "California", "Tennessee", "Texas")) %>%
...
pk12ed_mean_by_state %>%
  slice_max(mean, n=10) %>%
  ggplot(aes(x=reorder(state, -mean), # use the negative sign for the reverse order
             y=mean))+
  geom_col()
```
