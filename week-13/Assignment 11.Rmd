...
```{r}
v16 <- load_variables(2022, "acs5/profile", cache = TRUE) #Change this to 2022

broadband <- v16|>
  filter(name == "DP02_0154P")
```
...
pct_with_bb <-
    get_acs(
        geography = "school district (unified)", # geography that we're focusing on
        variables = "DP02_0154P", # variable from Step 3
        state = "TN",
        geometry = TRUE
    )
```
