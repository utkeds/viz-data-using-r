...
## Understanding projections{.smaller}
<hr>
- There are many, many [different projections](https://spatialreference.org/ref/epsg/)
- Reflect different purposes and trade-offs, different regions

## Understanding projections
<hr>
- What [projection](https://spatialreference.org/ref/epsg/2915/) is the file in?
```{r}
...
## Creating sf objects{.smaller}
<hr>
- all of the map components/layers must be in the same projection (matching the `.shp` file)
```{r}
#transforming the map projection of knox_sites_sf to match the knox_zones crs
...
<hr>
- Useful note: If the following conditions are met:
- You have an sf object with a given projection
- You have a data frame with a common variable to join on
- **You can `left_join()` them together** because {sf} re-projects the dataframe to match the crs of the sf object.
