...
- Assign a different fill color to each state. fill = as.factor(NAME)
...
```{r join_oz}
#CHANGE: Make variable names the same
ozmap_states_sf <-ozmap_states_sf |>
  left_join(oz_pop, by = c("NAME"="State/Territory"))
```

### 7. Final plot
- Plot the new joined data. (Case #2)
- Fill each state with the new `Population` variable. (Case #2)
- Make the outlines of each state white.
- Use `geom_point()` to plot the capital city of each state. (Case #1)
