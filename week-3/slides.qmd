...
# Weekly Check In, Download `.zip`, Open Folder in Positron
...
## `read.csv()` versus `read_csv()` {.smaller}
<hr>
:::{layout="[50,50]"}
:::{.column}
#### `read.csv()`
...
## Double-Colon `::` Operator {.smaller}
<hr>
- At least three typical use cases for `::`
  1. Don't want to load full package
      - Not: `library(janitor)` for just `clean_names()`
      - Instead: `janitor::clean_names()`

## Double-Colon `::` Operator {.smaller}
<hr>
  2. Avoiding package conflicts
     ![](slides_images/tidyverse_conflicts.png)
      - Both `dplyr` and `stats` packages have `filter()`
      - Lets you specify which `filter()` function to use
  3. Clarity of code
      - Makes it clear which function comes from which package
...
starwars |>
  group_by(species) |>
  summarize(n = n(), proportion = n / nrow(starwars) * 100)
```
:::
...
starwars |>
  group_by(species) |>
  summarize(
    med_height = median(height, na.rm = TRUE),
...
- I will float and answer questions. This one is more in-depth.
- Resources:
    - Week 2 slides
    - Week 3 slides
    - Beck reading
...
surv_rates_age_class$Pclass <- plyr::mapvalues(
  surv_rates_age_class$Pclass,
  from = c(1, 2, 3),
...
surv_rates_age_class |>
  ...
  labs(
    title = "Survival Rates by Class for Children",
    x = "Class",
    y = "Survival Rate"
  )
```
