...
## Having Trouble Rendering?
<hr>
- There has been some trouble rendering documents
  - Particularly happening with backticks that form the bounds of code cells
  ![](slides_images/linebreak_after_codecell.png)
  - How to fix?
    - Make sure there is a blank line after each code cell (hit enter)
...
`geom_vline()`
```{r, fig.height=4}
mean_highered <- tidykids_wide |>
  summarize(m = mean(highered, na.rm = T)) |>
  pull(m)

tidykids_wide |>
  ggplot(aes(x = highered, y = PK12ed)) +
  geom_vline(xintercept = mean_highered, color = "red") +
  geom_point()
```
...
`geom_hline()`
```{r, fig.height=4}
mean_pk12ed <- tidykids_wide |>
  summarize(m = mean(PK12ed, na.rm = T)) |>
  pull(m)

tidykids_wide |>
  ggplot(aes(x = highered, y = PK12ed)) +
  geom_hline(yintercept = mean_pk12ed, color = "blue") +
  geom_point()
```
...
# Amounts, Distributions, or Line?

## Amounts, Distributions, or Line?
<hr>
![Beigel, F et al. (2025). The drain of scientific publishing. *arXiv.* https://doi.org/10.48550/arXiv.2511.04820](slides_images/amounts_distributions_line.png)
