...
font_add(
  "DIN Condensed",
  regular = "/System/Library/Fonts/Supplemental/DIN Condensed Bold.ttf"
)
...
- Cairo (2013) discussion/ Group activity
- Looking ahead
...
library(ussf)
library(sf)
us_counties18sf_tn <- subset(
  boundaries(geography = "county"),
  STATEFP %in% c("47")
)
us_counties18_easttn <- subset(
  us_counties18sf_tn,
  COUNTYFP %in%
    c(
      "091",
...
df <- read_sheet(
  "https://docs.google.com/spreadsheets/d/1wFWUV3yH8qKzDDV4xop7L0YIxVC2_dXAPj2-5QQPR7Q/edit?gid=201277961#gid=201277961"
) |>
  select(c(9:14))
...
how_often_graphs <- df |>
  ...
  mutate(
    ...
    Question = fct_relevel(
      Question,
      c(
        "How often do you currently use each of the following? [Statistical software with a point-and-click graphical user interface (e.g., SPSS, SAS)]",
    ...
    Question = fct_recode(
      Question,
      "<b style='color:#2D912A; font-size:7.5pt;'>Point-and-Click Software:</b><span style='font-size:7.5pt;'> SPSS, SAS, etc.</span>" = "How often do you currently use each of the following? [Statistical software with a point-and-click graphical user interface (e.g., SPSS, SAS)]",
  ...
  labs(title = "How often do you use...?", x = NULL, y = NULL) +
  theme_minimal() +
...
level_of_confidence_graphs <- df |>
  ...
  mutate(
    ...
    Question = fct_relevel(
      Question,
      c(
        "Please rate your level of agreement with the following statements. [While working on a data analysis project, if I got stuck, I can find ways of overcoming the problem.]",
    ...
    Question = fct_recode(
      Question,
      "<span style='font-size:7.5pt;'>...confident in </span><b style='color:#28AF7F; font-size:7.5pt;'>Overcoming Problems</b>" = "Please rate your level of agreement with the following statements. [While working on a data analysis project, if I got stuck, I can find ways of overcoming the problem.]",
  ...
  geom_hline(yintercept = 0, color = "black", linewidth = 1.5) +
  labs(title = "To what degree are you...?", x = NULL, y = NULL) +
  theme_minimal() +
  scale_fill_manual(values = c("#28AF7F", "#32648E", "#481568")) +
  scale_y_continuous(
    expand = expansion(mult = c(0, 0)),
    limits = c(0, max_count_2),
    breaks = seq(0, max_count, by = 2)
  ) +
  theme(
    text = element_text(family = "Roboto Condensed"),
...
<hr>
:::{layout="[1,1], [1,1], [1,1]"}
- Cross-Country Runners & Triatheletes
- Crosstitcher
- Multilingual
- New Chess Player
- Familial Stuffed-Crust Legacy
- Travelers
- Teachers
- Vol Families
- Music-makers
...
![](slide_images/cross_stitch.jpeg){width=10%}
:::
...
- If there is a discrepancy between the syllabus and Canvas, Canvas takes precedence.
  - Slack me to let me know of any discrepancies, please!

## Nature of Discussions & Readings {.smaller}
<hr>
- Readings vary in nature
  - Some are websites/labs
  - Others are chapters
- Online discussions are scattered throughout the course and denoted by *** in the syllabus.

## AI Policy {.smaller}
<hr>
- *Mixed use* policy (as per UTK policy)
  - AI may be used to analyze and troubleshoot code *after* you have already manually written it.
- Include acknowledgement in each question where you used AI (either as a comment or text outside of code block):
  - “I acknowledge the use of [Generative AI Tool Name: Claude/Gemini/ChatGPT/etc.] in the writing of this code. Specifically, I encountered the following issue: [state problem addressed here], and with Generative AI's help, I learned to resolve it by [state how problem addressed here].”

## Canvas Review {.smaller}
<hr>
- [Canvas](https://utk.instructure.com/courses/261125) = Primary platform for this course.
...
- Many ways to get help:
  ...
  2. [UT AI Hub](https://oit.utk.edu/ai/aihub/) (see syllabus for policy)
  3. Email

## Why Positron/.Qmd and not RStudio/.Rmd {.smaller}
<hr>
- Posit (the parent company of both) is moving toward Positrong and away from RStudio
- Hadley Wickham says that new R programmers should not be taught RStudio
  - RStudio is currently being maintained, but unclear how long
- The team that built/maintained .Rmd was let go around 2025

## Class File Storage {.smaller}
<hr>
- If you haven't already, let's create a folder for this class.
- In [Canvas](https://utk.instructure.com/courses/261125) there is a .zip file for `week-1` with all the files you need.
...
- You save an R script and submit it as it is (.R file)
  - Not true with Quarto Markdown (.qmd) files!!
    - We'll work with these next week.
...
:::{column}
![](slide_images/quarto_raw.png)
:::

:::{column}
![](slide_images/quarto_rendered.png){.fragment}
:::
:::
...
::: {.callout-tip}
- Quarto also supports other languages like Python, but we won't use them in this class.
:::
...
## Breakout Rooms & Plot Critique {.smaller}
<hr>
Using your breakout room #, go to the corresponding Google Doc linked in Canvas.
...
## "Reading" in this class... {.smaller}
<hr>
- I use different verbs in the modules to describe the reading:

  - Read (traditional reading --> solidly skim)
  - Skim (quick reading)
  - Work through (this is code --> work through exercises)
  - Review (quickly browse --> reference material)
