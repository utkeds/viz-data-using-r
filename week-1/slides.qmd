...
```{r tn-map, warning=FALSE, message=FALSE, echo=FALSE}
#| fig-cap:
...
- If you want plain text, it needs to be as a comment
    - Comments start with a single hash (#) in R
- You save an R script and submit it as it is
  - Not true with Quarto Markdown files
...
:::{.small-font}
- Quarto Markdown is a file format for making **dynamic documents with R**.
- A Quarto Markdown document is written in markdown (a **plain text** format) and contains **chunks of embedded R code**.
...
:::{layout="[50,50]"}
![](QmdPlainTextExample.png)<br>[Plain Text in `.Qmd`]{.small-font}

![](MarkdownPlainTextFormatting.png)<br>[Plain Text Formatting]{.small-font}

:::

[- Note: verbatim formatting is created with backtick (tilde) key, not single quotation]{.extra-small-font}

## Quarto - Code Chunks (R){.smaller}
<hr>

- Another part of a Quarto Markdown document is the **code chunks**.

:::{layout="[50,50]"}
![](QMarkdownCodeChunkExample.png)<br>[Code Chunk in a `.Qmd`]{.small-font}


- This is a code chunk in Positron.
...
- You can use chunk options in the header (called YAML) of a `.Qmd` to establish defaults for the full document.
<br>
...
- **message**: Whether to show messages in the output document
- **eval**: Whether to run the code in the chunk
:::


## Quarto - Code Chunks (R){.smaller}
<hr>
- The default options will be applied to all code chunks in the document, unless you override them.

- You override the defaults by adding options in the header of an individual code chunk.

![](Override_chunk_defaults.png)

::: {.callout-tip}
You can name the code chunks (makes for better error messages). I named this one "test-block".
:::


## Quarto - Rendering Errors {.smaller}
<hr>
- When Quarto docs fail to render, it will tell you which chunk the error is in:
  - Note: I had named this chunk "error-chunk".

![](CodeChunkRenderError.png)<br>[Error in lines 29-31]{.small-font}

## Quarto {.smaller}
<hr>
- Most of your homework will be submitted as a Quarto Markdown files.
- Starting next week...
- We focus on `.html` outputs from your Quarto documents because Canvas can read them and they support interactive plots

## Quarto Markdown - Plain Text - Activity {.smaller}
<hr>
- In [Canvas](https://utk.instructure.com/courses/213032) you will find an activity called `Quarto Markdown: In-Class Activity`
- It should open up another page with instructions.
- In the `week-1` folder you will find the file for the activity called `Quarto Markdown: In-Class Activity`.
