...
```{r}
us_rent_income %>% 
  mutate(prop_of_income = rent_yearly / income) %>% 
  mutate(NAME = ifelse(prop_of_income < .50, NA, NAME)) %>% 
  ggplot(aes(x = income, y = rent_yearly, label = NAME)) +
  geom_point() +
  geom_text(hjust = -.25)
```
