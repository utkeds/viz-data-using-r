...
This is quite open-ended; there are many different sheets (individual files) within the Excel workbook. Which you use is up to you. *Note*: `read_xlsx` and `read_excel` take a sheet argument. Code to prepare the 2020 (the last sheet) and national rankings (the first) are provided in this zip file.
...
d <- read_csv("us-news-rankings-2020-processed.csv")
...
2. Use custom color palette in at least one visualization
3. Use `theme_minimal` or another theme
4. Have a title, using `labs()`
