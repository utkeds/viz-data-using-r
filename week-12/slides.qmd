...
## alt tags in .qmd {.smaller}
<hr>
- Last week, there was a question about creating alt tags in a .qmd file
- This is *optional* for your final project.
- When you create your code cell, you can add an alt tag there

```r
#| fig-alt: "This is alt text."
```

## Sizing Composite Plots {.smaller}
<hr>
- Recall you can edit the size of the cell output (inches):
...
- You can also manually [set the vertical and horizontal proportions](https://patchwork.data-imaginist.com/articles/guides/layout.html) of the composite plot in {patchwork}
...
## Preview vs. Rendered vs. ggsave() {.smaller}
<hr>
- RStudio handled this better than Positron
- One suggestion: Explicitly set the size of the plot in the code cell and in ggsave 
  - *both in inches*
- Don't go by plot preview pane
- Render early and often and go by rendered preview

## But what size do I want? {.smaller}
<hr>
**A Hack**

```r
# Pop-up: (call in console)
quartz() #MacOS ONLY
...
# Get dimensions of pop-up (call in console)
dev.size("in") #Use these for your cell output specifications (fig-height, fig-width) and ggsave

ggsave("plotname.png", width = ?, height = ?, units = "in")
...
:::{column}
![](slides_images/1_Basic_gt_table.png)
:::
...
:::{column}
![](slides_images/2_Row_to_stub.png)
...
:::{column}
![](slides_images/3_Row_group_label.png)
...
## The `gt()` Function{.smaller}
<hr>
- Note that we can also use `dplyr::group_by()` to add *row group labels*
...
## The `gtsave()` Function{.smaller}
<hr>
- `gtsave()` helps us save our gt tables to a variety of different file types
    - .html
    - .pdf
    - .png
    - LaTeX
...
```r
gtcars |>
  select(mfr, model, msrp) |> #Making a small table for slide
  slice(1:5) |> #Making a small table for slide
  gt() |>
  tab_header(
...
![](slides_images/4_Table_header.png)
:::
...
```r
gtcars |>
  select(model, year, hp, trq) |> #Making a small table for slide
  slice(1:5) |> #Making a small table for slide
...
gtcars_small <- 
  ...
  ) |> #Making a small table for slide
  dplyr::slice(1:8) #Making a small table for slide

gtcars_small
```

![](slides_images/5_Table_spanner.png)
:::
...
gtcars_small |>
  ...
  tab_options(data_row.padding = px(2)) # Added to make table fit
```
:::
...
:::{column}
![](slides_images/7_Table_source_notes.png)
:::
...
sp500 |>
  ...
  tab_options(data_row.padding = px(3)) # Added to make table fit
```
:::
...
## `tab_options()` function{.smaller}
<hr>
- Change the padding of data rows to 3 pixels
```{r}
tab_1 |> 
  tab_options(data_row.padding = px(3)) # px() helper function used here
```
...
<hr>
- You have `Wk_12_Collab_Practice.qmd` in your .zip file
    - Uses {palmerpenguins} data (palmerpenguins::penguins)
