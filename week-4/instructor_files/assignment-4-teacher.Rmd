...
tidykids <- read_csv("canvas-student-files/week-4/tidykids.csv")
...
Create a scatterplot. For this plot, keep things simple. Use only a single state, one variable of your choice on the y-axis and year on the x. Note that in the next step, you'll extend this graph a bit. Note that the codebook is here: https://jrosen48.github.io/tidykids/articles/tidykids-codebook.html
...
tidykids |>
  ...
  ggplot()+
  geom_point(aes(x=year,y=inf_adj_perchild))
...
tidykids |>
  filter(state == "Tennessee" & variable == "PK12ed" & year >= 2005)|>
  ggplot()+
  geom_point(aes(x=year,y=inf_adj_perchild))+
  labs(x="Year", y="Inflation Adjusted Spending Per Child (by $1,000s)") +
  theme_minimal()
...
This may be more of a challenge. Consulting the ggplot2 cheat sheet (see Canvas) as a support, create a histogram for a single variable. Change one of the axis labels and add a title and/or subtitle, again using the `labs()` function.
...
tidykids |>
  ...
  geom_smooth(aes(x=year,y=inf_adj_perchild))+
  labs(x="Year", y="Inflation Adjusted Spending Per Child (by $1,000s)") +
  theme_minimal()
...
# Looks like TennCare spending per child is the highest on average in Tennessee.
```
