...
# Weekly Check In, Download `.zip`, Download Slides, Open Wk4 Folder
...
## Reminder of AI Policy on Syllabus
<hr>
- Remember, if you use AI on homework, you must use the language provided on the syllabus to explain how you used it.

- Please do not use AI on in-class work.
...
## Understanding the Pipe `|>` Operator {.smaller}
<hr>
- The pipe `|>` is used to chain functions together
- Think about it as a water pipe, it lets the data "flow" through the whole chain of functions you specify.
...
## {ggplot2} prework: Data Wrangling {.smaller}
<hr>
...
- In your `.zip` file, you have a `ggplot2-prework-data-wrangling.qmd` file. Open it.
  - We'll work through it in breakout rooms.
...
<br><br>
- **Please have the ggplot cheatsheet handy for the next slides.**
:::
...
p <- tidykids |>
  ...
  labs(
    ...
    x = "Per Pupil Spending in 2016 Dollars",
    y = "State"
  )
p
```
...
<hr>
- So, at a high level, there are three required steps to creating a `ggplot2` visualization:
...
1. Identify the relevant variables/calculations needed to answer RQ
2. Identify the `geoms`(look at [cheatsheet](https://opensource.posit.co/resources/cheatsheets/data-visualization/) if needed)
3. Draw a quick sketch of the graph I want to create.
3. Identify the data wrangling needed.
:::
:::
...
Identify the relevant variables and calculations needed to answer RQ
...
Identify the relevant [variables]{.scales-color} and calculations needed to answer RQ

:::{style="background-color: #e0e0e0; padding: 10px; border-radius: 5px;"}
- How do [U.S. regions]{.scales-color} compare in their average [inflation-adjusted per-child spending]{.scales-color} on public K-12 education in 2016? 
    - How do the regions compare to the national average?
:::
...
Identify the relevant [variables]{.scales-color} and [calculations]{.fountain-color} needed to answer RQ

:::{style="background-color: #e0e0e0; padding: 10px; border-radius: 5px;"}
- How do [U.S. regions]{.scales-color} compare in their [average]{.fountain-color} [inflation-adjusted per-child spending]{.scales-color} on public K-12 education in 2016? 
    - How do the regions compare to the national average?
:::

## Example 1: Pre-work
<hr>
- **Step 1:** Relevant [Variables]{.scales-color} & [Calculations]{.fountain-color}
...
1. U.S. Regions
2. Inflation-Adjusted Spending Per Child

## Example 1: Pre-work [Commercial Break]
<hr>
Understanding R Variable Types

:::{.smaller-font}

- Discrete
  - Factor (`Pclass` from Titanic)
  - Character (`name` from StarWars)
  - Logical (`Survived` from Titanic)
- Continuous
  - Numeric/Double (`mass` from StarWars)
  - Integer (`age` in Titanic) ***(Integers can also be discrete, often when only a few)
  - Dates/Times
...
1. U.S. Regions (Discrete)
2. Inflation-Adjusted Spending Per Child (Continuous)

## Example 1: Pre-work
<hr>
- **Step 2:** Identify `geom_`s with [cheatsheet](https://opensource.posit.co/resources/cheatsheets/data-visualization/)
...
## Example 1: Pre-work
<hr>
- **Step 3:** Choose `geom_`s and sketch
...
:::{column}
![](slides_images/practical_approach/excalidraw_sketch_cols.png)
...
:::{column}
![](slides_images/practical_approach/excalidraw_sketch_hline.png)
...
## Example 1: Pre-work
<hr>
- **Step 4:** Plan for Data Wrangling
...
:::{column}
  ...
  2. Filter `year` for 2016
  3. Filter `expenditure` for "PK12ed"
  4. Group by `region`
      - Summarize:
          - mean
  5. Calculate national mean (separate from tibble)
